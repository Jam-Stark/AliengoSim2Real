...
# 加载 mujoco 模型
m = mujoco.MjModel.from_xml_path('/home/albusgive2/go2w_sim2sim/robot/go2w_description/mjcf/scene_terrain.xml')
d = mujoco.MjData(m)
...
isaaclab_joint_names = ['FL_hip_joint', 'FR_hip_joint', 'RL_hip_joint',
                        ...
                        'FL_foot_joint', 'FR_foot_joint', 'RL_foot_joint',
                        'RR_foot_joint']
...
def get_obs(actions, default_dof_pos, commands=[0.3, 1.0, 0.0]):
    ...
    dof_pos = torch.zeros(16, device=device, dtype=torch.float32)
    for i in range(len(leg_joint_names)):
        dof_pos[i] = get_sensor_data(leg_joint_names[i]+"_pos")[0]
    dof_pos[12:16] = 0.0
...
def main():
    # 加载模型
    try:
        loaded_policy = torch.jit.load("/home/albusgive2/go2w_sim2sim/policy/policy.pt")
    ...
    # 初始化观察数据
    default_dof_pos_without_wheel = torch.tensor([
        0.00, 0.80, -1.50,
        0.00, 0.80, -1.50,
        0.00, 0.80, -1.50,
        0.00, 0.80, -1.50,
