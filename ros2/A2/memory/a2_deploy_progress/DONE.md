...
## 2026-06-08 21:27 HKT

- [x] `a2_lowlevel_smoke` 新增 `custom_pose` mode：`custom_pose_q` 必须 explicit exact 12 finite values，按 A2 low-level order 解释；首次 fresh LowState 后从 current q smooth interpolation 到 target，再用低刚度 `publish_joint_commands()` hold target。
- [x] `a2_real_robot_test.sh` 新增 guarded `pose-hold [duration]`：要求 `A2_ALLOW_POSE_LOWCMD=1` 和 explicit `A2_POLICY_STOP_POSE_Q`，缺失或格式错误会 fail fast；guard 不允许从 run config 取得。
- [x] 更新 `a2_policy_remote.env`、`A2_REAL_DEPLOY_RUNBOOK.md`、`A2_REAL_ROBOT_TEST.md` 和 `README.md`：记录 stop policy -> `no-lowcmd` -> optional `pose-hold` -> `no-lowcmd` -> `motion-restore` sequence；`Select` 仍是 immediate local stop，不自动执行 custom pose recovery。

## 2026-06-08 21:38 HKT

- [x] 实现 A2 active brake gate：`a2_policy_deploy` 新增 `brake_gate_enabled`、`brake_force_x_threshold`、`brake_min_cmd_vx`、`brake_max_abs_yaw`、`brake_hold_steps` 参数；裸 node 默认关闭，wrapper config 默认开启。
- [x] brake gate 在 `computeAction()` 后、`publish_joint_commands()` 前读取 aux dim 6 的 `pred_base_force_local[0]`，默认 unitless threshold `<= -0.6` 连续 `2` steps，且满足 `cmd_vx>=0.2`、`abs(cmd_yaw)<=0.10`、非 standing command 后 latch。
- [x] brake active 当前 tick 调用 `publish_zero()` / `set_zero_command()`，清零 `last_raw_action_`、`obs_actions[kPolicyId]` 和 `gait_phase_`，不发布 policy joint command；latch 到 command standing / eligibility lost / local stop / runtime reset。
- [x] wrapper/config/docs 已从 comment-only brake placeholder 升级为 active config，并说明 threshold 是 A2 observed unitless aux scale，不是 Newton；TODO 保留部署机/实机验证阈值、符号和稳定性。
