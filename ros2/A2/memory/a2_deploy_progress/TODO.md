...
- [ ] A2 brake gate 已改为 command override only，仍需部署机/实机验证阈值、符号和稳定性：
  ...
  - 验证 brake active 后下一轮 policy observation command override 为 `[0,0,0]`，且 release 判断基于 raw requested command；stick 回中 / command standing / not eligible / local stop 能 release。
