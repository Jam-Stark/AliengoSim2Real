...
scope: ros2/A2
status: active
last_updated: "2026-06-08 22:20 HKT"
owned_paths:
  - ros2/A2/
  - ros2/A2_Guide/
read_when:
...
- 2026-06-08 22:20 HKT 已将 A2 brake gate 从 zero LowCmd stop path 改为 command override only：默认裸 node `brake_gate_enabled=false`，wrapper run config 默认 `A2_POLICY_BRAKE_GATE_ENABLED=true`；active motion path 在 `computeAction()` 后读取 aux dim 6 layout `pred_base_force_local[0]`，默认用 unitless threshold `<= -0.6` 连续 `2` steps 且 raw requested `cmd_vx>=0.2`、`abs(cmd_yaw)<=0.10`、非 standing command 时 latch。触发当前 tick 不 `publish_zero()`、不切 LowCmd stop mode、不清 PD/last action/gait phase、不跳过 policy joint command；下一轮 observation 前仅把 policy observation command override 为 `[0,0,0]`，后续仍走 `computeAction()`、action validation 和 `publish_joint_commands()`。listen-only / `enable_motion=false` / `policy-aux-live` 仍不发布 LowCmd。
...
- brake gate 已改为 command override only 并在 wrapper config 默认启用；仍需部署机/实机用 `policy-aux-monitor` 对照 active `/a2/policy_aux` 验证 `pred_base_force_local[0] <= -0.6` 的符号、unitless threshold 裕量、2-step latch、no zero-LowCmd stop、normal PD command continues、command override/release 稳定性。
...
- 2026-06-08 22:20 HKT 已将 A2 brake gate 改为 command override only：brake latch 后下一轮 observation command override 为 `[0,0,0]`，不再 `publish_zero()`、不切 stop mode、不清 PD/last action/gait phase、不跳过 `publish_joint_commands()`；新增 raw requested command 作为 eligibility/release 来源，并同步更新 README、validation guide、runbook 和 memory。
